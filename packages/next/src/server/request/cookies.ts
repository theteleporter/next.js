 ...
 * synchronously to faciliate migration. The `DangerouslyUnwrapCookies` type is added to your code by a codemod that attempts to automatically
 ...
 * explicit type case to `DangerouslyUnwrapCookies` to enable typescript to allow for the synchronous usage only where it is actually necessary.
 *
 ...
 *
 * You can find intsances that require manual migration by searching for `DangerouslyUnwrapCookies` in your codebase or by search for a comment that
 * starts with:
 *
 * ```
 ...
 * without awaiting the return value first. When this change happens the type `DangerouslyUnwrapCookies` will be updated to reflect that is it no longer
 * usable.
 *
 ...
 * @deprecated
 */
export type DangerouslyUnwrapCookies = ReadonlyRequestCookies
