 ...
 * synchronously to faciliate migration. The `DangerouslyUnwrapHeaders` type is added to your code by a codemod that attempts to automatically
 ...
 * explicit type case to `DangerouslyUnwrapHeaders` to enable typescript to allow for the synchronous usage only where it is actually necessary.
 *
 ...
 *
 * You can find intsances that require manual migration by searching for `DangerouslyUnwrapHeaders` in your codebase or by search for a comment that
 * starts with:
 *
 * ```
 ...
 * without awaiting the return value first. When this change happens the type `DangerouslyUnwrapHeaders` will be updated to reflect that is it no longer
 * usable.
 *
 ...
 * @deprecated
 */
export type DangerouslyUnwrapHeaders = ReadonlyHeaders
