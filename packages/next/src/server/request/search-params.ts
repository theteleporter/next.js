 ...
 * The `DangerouslyUnwrapParams` type is available if you need to temporarily access the underlying params without first awaiting or `use`ing the Promise.
 *
 ...
 *
 * ```tsx
 * type MyParams = Promise<{ foo: string }>
 *
 * export default async function Layout({ params }: { params: MyParams }) {
 *  const directParams = params as unknown as DangerouslyUnwrapParams<typeof params>
 *  return ...
 * }
 * ```
 *
 ...
 * @deprecated
 */
export type DangerouslyUnwrapSearchParams<P> =
  P extends Promise<infer U> ? U : never
